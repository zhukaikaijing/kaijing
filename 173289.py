...
# 得物森林
# export dewu_x_auth_token1='Bearer ey**&Bearer ey**',多账号使用换行或&
# export dewu_sk1='9MFyPaKgdQl*********&9MFyPaKgdQl*********' 多账号使用换行或& 一个账号对应一个sk，顺序需与dewu_x_auth_token1相同，相同的sk不同账号使用会出验证码，同一设备sk相同，建议一个设备登一个账号抓包
# export dewu_user_agent1='*****pp/5.25.0******&*****pp/5.25.0******' 多账号使用换行或&  同一 sk 对应的 user_agent，其中需含有得物版本号 版本号推荐不小于 5.24.5
...
def get_env():
    ...
    env_str = os.getenv("dewu_x_auth_token1")
    if env_str:
        ck_list += env_str.replace("&", "\n").split("\n")
    env_str = os.getenv("dewu_help_signal")
    if env_str:
        HELP_SIGNAL = env_str
    env_str = os.getenv("dewu_sk1")
    if env_str:
        sk_list = env_str.replace("&", "\n").split("\n")
    env_str = os.getenv("dewu_user_agent1")
    if env_str:
...
def main():
    ...
    if not ck_list:
        ...
        return
    if not sk_list:
        myprint('dewu_sk1 未填写！')
        return
    if not user_agent_list:
        myprint('dewu_user_agent1 未填写！')
        return
    ...
    if ck_count != sk_count:
        print(
            f'dewu_x_auth_token1({ck_count}个)与dewu_user_agent1({user_agent_count}个)数量不相等')
        return
    if ck_count != sk_count:
        print(
            f'dewu_x_auth_token1({ck_count}个)与dewu_sk1({sk_count}个)数量不相等')
        return
